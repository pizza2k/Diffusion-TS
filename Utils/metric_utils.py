...
def visualization(name, ori_data, generated_data, analysis, compare=3000):
    """Using PCA or tSNE for generated and original data visualization.
  
    Args:
        - ori_data: original data
        - generated_data: generated synthetic data (can be None)
    ...
    # 检查是否有生成数据
    has_generated = generated_data is not None
    ...
    if analysis == 'pca':
        ...
        if has_generated:
            ...
            plt.scatter(pca_results[:, 0], pca_results[:, 1],
                        c=colors, alpha=0.2, label="Original")
            ax.legend()
        ...
        plt.ylabel('y_pca')
        #plt.show()
        plt.savefig(f'pic/pca_plot_{name}.png', dpi=300, bbox_inches='tight')
        ...
        plt.ylabel('y_tsne')
        #plt.show()
        plt.savefig(f'pic/tsne_plot_{name}.png', dpi=300, bbox_inches='tight')
        
    elif analysis == 'kernel':
        f, ax = plt.subplots(1)
        
        if has_generated:
            sns.distplot(prep_data, hist=False, kde=True, kde_kws={'linewidth': 5}, 
                        label='Original', color="red")
            sns.distplot(prep_data_hat, hist=False, kde=True, 
        ...
        # plt.show()
        plt.savefig(f'pic/kernel_plot_{name}.png', dpi=300, bbox_inches='tight')
        plt.close()
